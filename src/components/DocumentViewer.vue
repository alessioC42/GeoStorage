...
<template>
  <v-app>
    <v-app-bar color="secondary" style="border-radius: 5px">
      <v-toolbar-title style="font-size: large"
        >Stored Documents for your StoryPoint</v-toolbar-title
      >
      <v-spacer></v-spacer>
      ...
      <v-btn
        icon="mdi-download"
        @click="DataProvider.getInstance().downloadStoryArchive(storyID)"
      />
    </v-app-bar>
    <v-sheet style="height: 100%; margin-top: 85px; overflow-y: scroll; overflow-x: hidden">
      <v-row style="padding-right: 10px">
        ...
        <FileCard
          ...
          v-else
        />
        <div class="center-screen" v-if="remoteFiles.length === 0 && !isLoading">
