<template>
  <v-card class="file-card">
    <v-img :src="thumbnail" aspect-ratio="1.8" class="grey lighten-2">
    ...
    <v-card-subtitle
      >{{ formatBytes(file.filesize) }} - {{ getTimeCreatedString() }}</v-card-subtitle
    >
...
export default defineComponent({
  ...
  methods: {
    ...
    loadThumbnail() {
      ...
      if (['jpg', 'jpeg', 'png', 'gif', 'webp', 'svg', 'tiff'].includes(extension)) {
        DataProvider.getInstance()
          .getThumnnailBase64Image(this.storyPointId, this.file._id)
          .then((base64Image) => {
    ...
    async downloadFile() {
      ...
      await DataProvider.getInstance().downloadStoryFile(
        this.storyPointId,
        this.file._id,
        this.file.filename
      )
    ...
    async renameFile(newFileName: string) {
      await DataProvider.getInstance().renameStoryFile(
        this.storyPointId,
        this.file._id,
        newFileName
      )
