...
export class DataProvider {
  ...
  static getInstance(): DataProvider {
    if (!DataProvider.instance) {
      DataProvider.instance = new DataProvider('https://geostorage.duckdns.org')
    }
  ...
  async getAllStories(): Promise<storyPoint[]> {
    ...
    const result: storyPoint[] = (await response.json())['storypoints']
    this.stories.value = []
  ...
  async getEntireStoryPoint(id: string): Promise<storyPoint | null> {
    try {
      const response = await this.fetch(
        `${this.baseURL}/api/company/${this.companyID}/storypoints/${id}`
      )
      if (response.status == 200) {
  ...
  async createStoryPoint(
    ...
    try {
      const response = await this.fetch(
        `${this.baseURL}/api/company/${this.companyID}/storypoints`,
        {
  ...
  async searchStoryPointsRemote(query: string): Promise<storyPoint[]> {
    try {
      const response = await this.fetch(
        `${this.baseURL}/api/company/${this.companyID}/storypoints/search?q=${query}`
      )
      if (response.status == 200) {
  ...
  async searchStoryPointsByDistanceRemote(coords: LatLngTuple): Promise<storyPoint[]> {
    try {
      const response = await this.fetch(
        `${this.baseURL}/api/company/${this.companyID}/storypoints/nearby?lat=${coords[0]}&lng=${coords[1]}`
      )
      if (response.status == 200) {
  ...
  async updateStoryPoint(
    id: dbID,
    { title, description, history }: { title: string; description: string; history: historyItem[] }
  ): Promise<void> {
    try {
      await this.fetch(`${this.baseURL}/api/company/${this.companyID}/storypoints/${id}`, {
        method: 'PUT',
  ...
  async getAllFiles(storyPoint: string): Promise<remoteFile[]> {
    try {
      const response = await this.fetch(
        `${this.baseURL}/api/company/${this.companyID}/storypoints/${storyPoint}/files`
      )
      if (response.status == 200) {
  ...
  async uploadFile(file: Blob, filename: string, storyPointID: string): Promise<void> {
    ...
    await this.fetch(
      `${this.baseURL}/api/company/${this.companyID}/storypoints/${storyPointID}/files?filename=${encodeURIComponent(filename)}`,
      {
        method: 'POST',
        body: formData
      }
  ...
  async downloadStoryArchive(storyID: string): Promise<void> {
    try {
      await this.downloadFile(
        `${this.baseURL}/api/company/${this.companyID}/storypoints/${storyID}/files/archive`,
        'archive.zip'
      )
    } catch (e) {
      console.error(e)
  ...
  async downloadStoryFile(storyID: string, fileID: string, filename: string) {
    try {
      await this.downloadFile(
        `${this.baseURL}/api/company/${this.companyID}/storypoints/${storyID}/files/${fileID}`,
        filename
      )
    } catch (e) {
      console.error(e)
  ...
  async renameStoryFile(storyID: string, fileID: string, newName: string): Promise<void> {
    try {
      await this.fetch(
        `${this.baseURL}/api/company/${this.companyID}/storypoints/${storyID}/files/${fileID}/rename`,
        {
          method: 'PUT',
  ...
  async getThumnnailBase64Image(storyID: string, fileID: string): Promise<string> {
    try {
      const response = await this.fetch(
        `${this.baseURL}/api/company/${this.companyID}/storypoints/${storyID}/files/${fileID}/thumbnail`
      )
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`)
      }
